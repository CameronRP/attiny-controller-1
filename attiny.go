...
const (
	// Registers. 0x00 to 0x10
	versionReg Register = iota
	typeReg
	cameraStateReg
	resetWatchdogReg
	triggerSleepReg
	triggerSleepDelayReg
	cameraWakeUpReg
	sleepDurationMSBReg
	sleepDurationMIDReg
	sleepDurationLSBReg
...
func updateATtinyFirmware() error {
	...
	log.Println("Erasing device.") //TODO check if it erases EPROM
...
func (a *attiny) PingWatchdog() error {
	return a.writeRegister(resetWatchdogReg, 0x01)
}
...
//TODO This logic will need to be updated to work with the hotspot stuff
func (a *attiny) UpdateWifiState() error {
	...
	wifiConnected := strings.Contains(string(outByte), "state UP")
	if a.wifiConnectedState == wifiConnected {
		return nil
	}

	var newCameraState CameraState = 0
	if wifiConnected {
		newCameraState = stateConnectedToNetwork
	} else {
		newCameraState = statePoweredOn
	}
	return a.WriteCameraState(newCameraState)
}
